...
if (isset($_GET['hapus'])) { // Cek jika ada parameter "hapus" di URL
    ...
    mysqli_query($conn, "DELETE FROM informasi_medis WHERE id=$id"); // Jalankan query untuk menghapus data dari database berdasarkan id
...
if (isset($_GET['edit'])) { // Cek jika ada parameter "edit" di URL
    ...
    $result = mysqli_query($conn, "SELECT * FROM informasi_medis WHERE id=$editIndex");
    $dataEdit = mysqli_fetch_assoc($result); // Ambil data yang akan diedit dari database berdasarkan id

    if ($dataEdit) {
        $judul = $dataEdit['judul']; // Set variabel judul dengan data yang diambil dari database untuk ditampilkan di form
        $kategori = $dataEdit['kategori']; // Set variabel kategori dengan data yang diambil dari database untuk ditampilkan di form
        $deskripsi = $dataEdit['deskripsi']; // Set variabel deskripsi dengan data yang diambil dari database untuk ditampilkan di form
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { // Cek jika form disubmit dengan metode POST
    ...
    if (!empty($judulPost) && !empty($deskripsiPost)) {

        if (!empty($indexPost)) {
            // UPDATE
            mysqli_query($conn, "
                UPDATE informasi_medis
                SET judul='$judulPost', kategori='$kategoriPost', deskripsi='$deskripsiPost'
                WHERE id=$indexPost
            "); // Jalankan query untuk memperbarui data di database berdasarkan id
        } else {
            // INSERT
            mysqli_query($conn, "
                INSERT INTO informasi_medis (judul, kategori, deskripsi)
                VALUES ('$judulPost', '$kategoriPost', '$deskripsiPost')
            "); // Jalankan query untuk menambahkan data baru ke database
        }
    }

    header("Location: " . $_SERVER['PHP_SELF']); // Redirect ke halaman yang sama untuk memperbarui tampilan setelah penyimpanan
...
<tbody>
<?php
$result = mysqli_query($conn, "SELECT * FROM informasi_medis");
