...
session_start();
include "koneksi.php";
// Data awal laporan keuangan yang akan ditampilkan di tabel
// array ini berisi beberapa contoh transaksi dengan tanggal, keterangan, jenis (pemasukan/pengeluaran), dan jumlah
// yang akan di tampilkan pertama kali pada tabel laporan keuangan sebelum ada input manual dari pengguna
$dataLaporan = [
...
// Proses input laporan manual dari form
// Jika user mengisi form dan menekan tombol simpan,
// maka data dikirim menggunakan metode POST
// Data yang dikirim melalui POST akan terlihat di URL
if(isset($_POST['tanggal'])){

// Mengambil data dari form inputan laporan manual yang dikirim melalui POST
    ...
    // Mengecek apakah ada id yang dikirim melalui form, jika ada berarti ini adalah update data
    // Jika tidak ada id, berarti ini adalah input data baru (insert)
    if(!empty($_POST['id'])){
        // UPDATE
        $id = $_POST['id'];
        // Menyiapkan statement SQL untuk memperbarui data laporan berdasarkan id dan user_id
        $stmt = $conn->prepare("UPDATE laporan SET tanggal=?, keterangan=?, jenis=?, jumlah=? WHERE id=? AND user_id=?");
        // Mengikat parameter ke statement SQL, dengan tipe data yang sesuai (s untuk string, i untuk integer)
        $stmt->bind_param("sssiii", $tanggal, $keterangan, $jenis, $jumlah, $id, $user_id);
    } else {
        // INSERT
        // Menyiapkan statement SQL untuk memasukkan data laporan baru ke dalam tabel laporan
        $stmt = $conn->prepare("INSERT INTO laporan (user_id, tanggal, keterangan, jenis, jumlah) VALUES (?, ?, ?, ?, ?)");
        // Mengikat parameter ke statement SQL, dengan tipe data yang sesuai (s untuk string, i untuk integer)
        $stmt->bind_param("isssi", $user_id, $tanggal, $keterangan, $jenis, $jumlah);
    }

    // Menjalankan statement SQL yang sudah disiapkan
    $stmt->execute();

    // Setelah data berhasil disimpan atau diperbarui, redirect kembali ke halaman yang sama untuk menampilkan data terbaru
    header("Location: ".$_SERVER['PHP_SELF']);
    exit;
}

// Mengecek apakah pengguna sudah login dengan memeriksa apakah session user_id sudah diset
if (!isset($_SESSION['user_id'])) {
    header("Location: TugasPraktikum1login.php");
}

// Proses hapus data laporan
// Jika ada parameter 'hapus' yang dikirim melalui URL, maka proses penghapusan data laporan berdasarkan id yang dikirim
if(isset($_GET['hapus'])){
    $id = $_GET['hapus'];

    // Menyiapkan statement SQL untuk menghapus data laporan berdasarkan id dan user_id
    $stmt = $conn->prepare("DELETE FROM laporan WHERE id=? AND user_id=?");
    // Mengikat parameter ke statement SQL, dengan tipe data yang sesuai (i untuk integer)
    $stmt->bind_param("ii", $id, $_SESSION['user_id']);
    $stmt->execute();

// Setelah data berhasil dihapus, redirect kembali ke halaman yang sama untuk menampilkan data terbaru
    header("Location: ".$_SERVER['PHP_SELF']);
    exit;
}

// Proses edit data laporan
$editData = null;

// Jika ada parameter 'edit' yang dikirim melalui URL, maka proses pengambilan data laporan berdasarkan id yang dikirim untuk ditampilkan di form input
// Data yang diambil akan digunakan untuk mengisi form input sehingga pengguna bisa melihat data yang akan diedit
if(isset($_GET['edit'])){
    $id = $_GET['edit'];
    ...
    $stmt->bind_param("ii", $id, $_SESSION['user_id']);
    $stmt->execute();
    // Mendapatkan hasil dari query yang dijalankan
    $result = $stmt->get_result();
    // Mengecek apakah query menghasilkan setidaknya satu baris data yang cocok dengan id dan user_id yang dikirim
