<!-- Tugas Praktikum 1 Register-Login menggunakan php yang sudah terhubung dengan database -->
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Jika semua input valid (tidak ada pesan error), maka data akan disimpan ke database
    // $conn->prepare() -> digunakan untuk menyiapkan query SQL dengan parameter yang akan diisi nanti
    // $stmt->bind_param() -> digunakan untuk mengikat parameter ke query SQL, "ssssss" menunjukkan bahwa semua parameter adalah string
    // $stmt->execute() -> digunakan untuk menjalankan query SQL yang sudah disiapkan
    // Jika query berhasil dijalankan, maka akan menampilkan alert bahwa registrasi berhasil dan mengarahkan ke halaman login
    // Jika query gagal dijalankan, maka akan menampilkan pesan error "Gagal daftar"
    if (
        $namaErr == "" && $emailErr == "" && $usernameErr == "" &&
        $passwordErr == "" && $teleponErr == "" && $alamatErr == ""
    ) {

        $stmt = $conn->prepare("INSERT INTO users (nama,email,username,password,telepon,alamat) VALUES (?,?,?,?,?,?)");
        $stmt->bind_param("ssssss", $nama,$email,$username,$password,$telepon,$alamat);

        if ($stmt->execute()) {
            echo "<script>alert('Registrasi berhasil'); window.location='1login.php';</script>";
        } else {
            echo "Gagal daftar";
        }
    }
...
<body>
        ...
        <form class="form-box" method="POST">
            ...
            <div class="input-card">
                <input type="text" name="email" placeholder="Masukkan email (contoh@gmail.com)"
                    ...
                    agar ketika terjadi error form tidak kosong kembali -->
                <span class="error"><?php echo $emailErr; ?></span> <!-- menampilkan pesan error jika email kosong -->
            </div>
            ...
            <div class="input-card">
                ...
                agar ketika terjadi error form tidak kosong kembali -->
                <span class="error"><?php echo $usernameErr; ?></span> <!-- menampilkan pesan error jika username kosong -->
            </div>

            <div class="input-card">
                <input type="password" name="password" placeholder="Buat password"> <!-- Menampilkan kembali data yang sudah diinput pengguna 
                agar ketika terjadi error form tidak kosong kembali -->
                <span class="error"><?php echo $passwordErr; ?></span> <!-- menampilkan pesan error jika password kosong -->
            </div>
            ...
            <div class="input-card">
                <input type="text" name="telepon" placeholder="Masukkan Nomor Telepon (08XXXXXXXXXX)"
                    ...
                    agar ketika terjadi error form tidak kosong kembali -->
                <span class="error"><?php echo $teleponErr; ?></span> <!-- menampilkan pesan error jika nomor telepon kosong -->
            </div>
            ...
            <div class="input-card">
                ...
                agar ketika terjadi error form tidak kosong kembali -->
                <span class="error"><?php echo $alamatErr; ?></span> <!-- menampilkan pesan error jika alamat kosong -->
            </div>
    ...
    // Jika semua kondisi terpenuhi, maka data yang sudah dimasukkan oleh pengguna akan ditampilkan dalam bentuk tabel 
    if (
        $_SERVER["REQUEST_METHOD"] == "POST" &&
        // Mengecek apakah semua pesan error kosong, yang berarti semua input valid 
        $namaErr == "" && $emailErr == "" && $usernameErr == "" && $passwordErr == "" && $teleponErr == "" && $alamatErr == ""
    ) {
