<?php
session_start();// Memulai sesi menyimpan status login
require 'koneksi.php'; // Menghubungkan ke database 

if(isset($_POST['login_btn'])){// Cek apakah tombol login ditekan
   
    $username = mysqli_real_escape_string($conn, $_POST['username']);// Mengamankan input username
    $password = $_POST['password'];// Password tidak perlu di-escape karena akan di-hash, bukan langsung disimpan

    $query = "SELECT * FROM ecotaste WHERE username = '$username'";// Query untuk mencari username di tabel 'ecotaste'
    $result = mysqli_query($conn, $query);// Eksekusi query

    if($result && mysqli_num_rows($result) === 1){// Cek apakah username ditemukan
        $row = mysqli_fetch_assoc($result);// Ambil data user dari hasil query
        
        if(password_verify($password, $row['password'])){// Cek apakah password yang dimasukkan cocok dengan hash di database
            $_SESSION['login'] = true;// Set status login
            $_SESSION['user'] = $username;// Simpan username di sesi
        ...
        $error = "Username tidak ditemukan di tabel!";
    }
}
?>
<!DOCTYPE html>
