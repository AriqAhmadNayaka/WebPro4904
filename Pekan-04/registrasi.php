...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Cek jika form disubmit dengan metode POST
    ...
    elseif (strlen($password) < 6) { // Validasi password minimal 6 karakter
        ...
        $hashed_password = password_hash($password, PASSWORD_DEFAULT); // Hash password menggunakan algoritma default (saat ini bcrypt)
    
        // Query insert ke database
        $query = "INSERT INTO users (role, name, email, password)
                    VALUES ('$role', '$name', '$email', '$hashed_password')"; // Query untuk menyimpan data pengguna baru ke tabel users di database
    
        // Jalankan query
        if (mysqli_query($conn, $query)) {
            $message = "Akun berhasil dibuat! Silakan login."; // Pesan sukses jika data berhasil disimpan ke database
            $type = "success"; // Tipe pesan untuk sukses
            header("refresh:2;url=login.php"); // Redirect ke halaman login setelah 2 detik
        } else {
            $message = "Gagal menyimpan data: " . mysqli_error($conn); // Pesan error jika terjadi masalah saat menyimpan data ke database
            $type = "danger"; // Tipe pesan untuk error
        }
    }
}
