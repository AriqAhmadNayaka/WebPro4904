...
<body>
  <!-- Dashboard admin: data dokter dikirim dari Admin::index -->
  <div class="admin-wrapper">
    <!-- SIDEBAR -->
    <aside class="admin-sidebar acrylic-card">
      <div class="profile-sidebar">
        <div class="avatar-placeholder">&#128100;</div>
        <h3 class="adminName"><?= $this->session->userdata('username') ?></h3>
        <p>Super Administrator</p>
      </div>

      <nav class="sidebar-nav">
        <a href="<?= base_url('index.php?c=admin&m=index') ?>" class="nav-item active">Manajemen Dokter</a>
      </nav>
    </aside>
        ...
        <div class="acrylic-card">
          <div
            style="
              ...
              ">
            <h4>Daftar Dokter</h4>
            <a href="<?= base_url('index.php?c=admin&m=create') ?>" class="btn-primary">
              + Tambah Dokter
            </a>
          </div>
          ...
          <table class="custom-table">
            ...
            <tbody id="doctorTableBody">
              <!-- Loop data dokter dari database -->
              <?php for ($i = 0; $i < count($dokter); $i++) : ?>
                <tr>
                  <td><?= $dokter[$i]['nama'] ?></td>
                  <td>Dokter <?= $text = isset($dokter[$i]['spesialisasi']) ? ucfirst($dokter[$i]['spesialisasi'] ?? '') : "" ?></td>
                  <td>
                  ...
                  <td>
                    <?php if (!empty($dokter[$i]['sertifikat'])) : ?>
                      <span class="status active" style="font-size: 0.75rem; cursor: pointer;" onclick="showCertificate('<?= base_url('uploads/') ?>' + <?= htmlspecialchars(json_encode($dokter[$i]['sertifikat'] ?? '')) ?>, <?= htmlspecialchars(json_encode($dokter[$i]['nama'] ?? '')) ?>)">&#128196; Lihat File</span>
                    <?php else : ?>
                  ...
                  <td class="action-cell">
                    ...
                    <a href="<?= base_url('index.php?c=admin&m=edit&id=' . $dokter[$i]['id']) ?>" class="btn-action edit">Edit</a>
  ...
  <script>
    ...
    function showDoctorDetail(nama, spesialis, email, telepon, sip, id, bio, cert) {
      ...
      if (cert) {
        ...
        certLink.onclick = () => showCertificate('<?= base_url('uploads/') ?>' + cert, nama);
      } else {
    ...
    function confirmDelete(nama, id) {
      customConfirm('Apakah Anda yakin ingin menghapus data untuk ' + nama + '?', function() {
        // Jika dikonfirmasi, arahkan ke route hapus dokter sesuai ID.
        window.location = '<?= base_url('index.php?c=admin&m=delete&id=') ?>' + id;
      });
    }
