...
require "Database.php"; //memanggil file koneksi database
require "Laporan.php"; //memanggil class Laporan OOP
include "Session.php";

$db = new Database(); //membuat objek database baru 
$laporan = new Laporan($db->conn); //kirim koneksi ke class laporan

// ambil session dari cookie jika belum ada
if(isset($_COOKIE['username']) && !isset($_SESSION['username'])){ 
    $_SESSION['username'] = $_COOKIE['username']; //set session dari cookie
}

if(!isset($_SESSION['username'])){ //cek jika user belum login
...
// ================= DELETE =================
if(isset($_GET['hapus'])){ //cek apakah ada parameter hapus
    $laporan->delete($_GET['hapus'], $username); //hapus data berdasarkan id

    echo "<script>
    alert('Data berhasil dihapus'); 
    window.location='dompet.php';
    </script>"; //notif dan reload halaman
}

// ================= EDIT =================
$editData = null; //default data edit kosong

if(isset($_GET['edit'])){ //cek apakah mode edit aktif
    $editData = $laporan->getById($_GET['edit']); //ambil data berdasarkan id
}

// ================= INSERT / UPDATE =================
if(isset($_POST['tanggal'])){ //cek apakah form dikirim

    if(isset($_POST['id']) && $_POST['id'] != ""){ //cek apakah update ada id
        $laporan->update(
            $_POST['id'], //id data yang akan di upload
            $_POST, //data form yang dikirim
            $_FILES['file'], //file di upload dari form
            $username, //username pemilik data
        ...
        window.location='dompet.php';
        </script>"; //notif update berhasil

    } else {
        $laporan->insert($_POST, $_FILES['file'], $username); //proses insert baru

        echo "<script>
        alert('Data berhasil ditambahkan'); 
        window.location='dompet.php';
        </script>"; //notif insert berhasil 
    }
}

// ================= TAMPIL DATA =================
$dataLaporan = $laporan->getAll($username); //ambil semua data milik user
?>
  ...
  <main class="main">
    ...
    <section id="section-laporan-manual" style="margin-top: 24px; scroll-margin-top: 80px">
      ...
      <div class="row">
        ...
        <form id="formManual" method="POST" enctype="multipart/form-data" action="">
        ...
        <div class="table-card" style="flex:1;">
          ...
          <tr>
              ...
              <td>
                <?php if($row['file']){ ?>
                  <a href="uploads/<?php echo $row['file']; ?>" target="_blank">Lihat</a>
                <?php } ?>
              </td>
              <td>
