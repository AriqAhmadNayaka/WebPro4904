<?php
session_start(); // memulai session untuk simpan data login
require "Database.php"; // memanggil class koneksi database OOP
require "Auth.php"; // memanggil class autentikasi user login

$db = new Database(); // membuat objek database untuk koneksi
$auth = new Auth($db->conn); // kirim koneksi ke class Auth

$username = ""; // inisialisasi variabel username kosong awal

// ambil dari cookie
if(isset($_COOKIE['username'])){ // cek apakah cookie username tersedia
    $username = $_COOKIE['username']; // isi username dari cookie browser
}

// kalau sudah login
if(isset($_SESSION['username'])){ // cek apakah user sudah login sebelumnya
    header("Location: dashboard.php"); // redirect ke halaman dashboard utama
...
// proses login
if(isset($_POST['login'])){ // cek apakah tombol login ditekan user
    $username = $_POST['username']; // ambil input username dari form
    $password = $_POST['password']; // ambil input password dari form

    if($auth->login($username, $password)){ // cek login ke database user

        echo "<script>
        alert('Login berhasil!'); // tampilkan notifikasi login berhasil
        window.location='dashboard.php'; // arahkan ke dashboard setelah login
        </script>";
    } else {
        $error = "Username atau password salah!"; // simpan pesan error login gagal
    }
}
?>
