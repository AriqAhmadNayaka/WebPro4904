...
// Muat UserModel — child class dari Database yang menangani semua logika user
// Sesuai modul 6.4.8: UserModel extends Database, mewarisi koneksi otomatis
require_once 'classes/../UserModel.php';

// Buat objek UserModel — konstruktor Database otomatis membuka koneksi
// Sesuai modul 6.4.5: object adalah hasil instansiasi dari class
$userModel = new UserModel();

// Daftar redirect sesuai role
$ROLE_REDIRECT = [
    'sekolah' => './sekolah/dashboard/dashboardSekolah.php',
    // role lain belum tersedia di praktikum ini
];

// Variabel notifikasi dan penanda section aktif
$notification = '';
$notification_type = '';
$active_section = 'login';

// CEK COOKIE "INGAT SAYA" — memanggil method bacaCookie() dari UserModel
// Sesuai modul 5.4.7: akses nilai cookie menggunakan $_COOKIE["nama"]
$cookie = $userModel->bacaCookie();
$cookieEmail = $cookie['email'];
$cookieRole = $cookie['role'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'register') {
        ...
        // Bersihkan input menggunakan method testInput() dari UserModel
        $name = $userModel->testInput($_POST['name'] ?? '');
        $email = $userModel->testInput($_POST['email'] ?? '');
        $password = $_POST['password'] ?? '';
        $role = $userModel->testInput($_POST['role'] ?? '');
        ...
        // Cek email sudah terdaftar via method cekEmailTerdaftar() dari UserModel
        } elseif ($userModel->cekEmailTerdaftar($email)) {
            $notification = 'Email sudah terdaftar';
            $notification_type = 'error';

        } else {
            // Simpan user baru via method register() dari UserModel
            $newId = $userModel->register($name, $email, $password, $role);
    ...
    // LOGIN
        ...
        if (empty($email) || empty($password) || empty($role)) {
            ...
            // Cari user via method cariUser() dari UserModel
            $user = $userModel->cariUser($email, $role);

            // Verifikasi password via method verifikasiPassword() dari UserModel
            if (!$user || !$userModel->verifikasiPassword($password, $user['password'])) {
                ...
                // COOKIE "INGAT SAYA" via method simpanCookie()/hapusCookie() dari UserModel
                // Sesuai modul 5.4.6: setcookie() dipanggil sebelum ada output HTML
                if (isset($_POST['ingat_saya'])) {
                    $userModel->simpanCookie($user['email'], $user['role']);
                } else {
                    $userModel->hapusCookie();
                }
...
// Destruktor UserModel otomatis menutup koneksi saat script selesai
            ...
            <form id="loginForm" class="auth-form" method="POST" action="auth.php">
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        ...
                        <!-- value diisi dari cookie via method bacaCookie() dari UserModel -->
                ...
                <div class="form-group">
                    ...
                    <div class="select-wrapper">
                        <!-- selected diisi dari cookie via $cookieRole -->
                ...
                <!-- Checkbox Ingat Saya jika dicentang, method simpanCookie() dipanggil -->
