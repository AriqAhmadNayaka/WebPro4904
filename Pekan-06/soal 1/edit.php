...
/**
 * 1. Flow Validasi & Pengambilan Data
 * Mengecek apakah ID valid dan datanya tersedia di database.
 */
...
if (isset($_POST['update'])) {
  ...
  /**
   * Handle Update File Sertifikat
   * Jika ada file baru, hapus yang lama dan simpan yang baru.
   */
  $fileUpdate = FileHelper::handleUpdate($_FILES['sertifikat'], $oldData['sertifikat']);
  
  if ($fileUpdate['status'] !== 'error') {
    $data['sertifikat'] = $fileUpdate['filename'];
    
    try {
      // 3. Update Database: Eksekusi perubahan ke DB
      $doctorModel->update($id, $data);
      $success_msg = "Data dokter berhasil diperbarui!";
      
      // Update variabel local agar form merefleksikan perubahan terbaru
