<?php
session_start();
// Memanggil file User.php yang berisi class User untuk mengelola proses registrasi dan login
...
if ($userObj->checkSession()) {
    // Jika pengguna sudah login, maka tidak perlu daftar lagi dan langsung diarahkan ke Dashboard
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Jika semua input sudah valid (tidak ada pesan error), maka proses registrasi akan dilakukan dengan memanggil method register dari class User
    if ($namaErr == "" && $emailErr == "" && $usernameErr == "" && $passwordErr == "" && $teleponErr == "" && $alamatErr == "") {
        
        // Memanggil fungsi register dari Class User
        if ($userObj->register($nama, $email, $username, $password, $telepon, $alamat)) {
        ...
        <form class="form-box" method="POST">
            ...
            <div class="input-card">
                <input type="text" name="telepon" placeholder="Masukkan Nomor Telepon (08XXXXXXXXXX)"
                    value="<?php echo $telepon; ?>" required> <!-- Menampilkan kembali data yang sudah diinput pengguna 
                    agar ketika terjadi error form tidak kosong kembali -->
