...
const LEGACY_UPLOADS_URL = 'http://localhost/PemprogramanWeb/WebPro4904/Pekan09/ci3_project/uploads/posts';

const getImageFilename = (imagePath = '') => imagePath.replace(/^posts\//, '');

const getFallbackImageUrl = (post) => {
    const filename = getImageFilename(post?.image);
    return filename ? `${LEGACY_UPLOADS_URL}/${filename}` : '';
};
...
const PostDetail = () => {
    ...
    const imageUrl = post?.image_url || getFallbackImageUrl(post);
                    ...
                    <article className="bg-white rounded-lg border border-gray-200 shadow-md overflow-hidden">
                        {imageUrl && (
                            <img
                                ...
                                onError={(e) => {
                                    const fallbackUrl = getFallbackImageUrl(post);
