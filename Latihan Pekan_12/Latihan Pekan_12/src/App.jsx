  ...
  return (
    <div className="app-container">
      ...
      {selectedId ? (
        ...
        <DaftarArtikel
          token={token}
          bukaDetail={(id) => setSelectedId(id)}
        />
      )}
    </div>
  );
...
function Login({ setToken }) {
  ...
  const prosesLogin = (e) => {
    ...
    axios
      .post('https://pekan12-webpro.nets.web.id/api_login.php', formData)
      .then((res) => {
...
function DaftarArtikel({ token, bukaDetail }) {
  ...
  useEffect(() => {
    axios
      .get('https://pekan12-webpro.nets.web.id/api_artikel.php', {
        headers: {
...
function DetailArtikel({ id, token, kembali }) {
  ...
  useEffect(() => {
    axios
      .get(`https://pekan12-webpro.nets.web.id/api_detail.php?id=${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  return (
    <div className="card-ui">
      ...
      <h1 className="section-title" style={{ border: 'none', padding: 0 }}>
        {post.title}
      </h1>

      <p className="detail-meta">
        <strong>Ditulis oleh:</strong> {post.author}
      </p>
