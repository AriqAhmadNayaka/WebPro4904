...
const Login = () => {
    // State lokal untuk menyimpan input email dari pengguna.
    const [email, setEmail] = useState('');
    // State lokal untuk menyimpan input password dari pengguna.
    const [password, setPassword] = useState('');
    // State lokal untuk menampung pesan kesalahan (jika login gagal).
    const [error, setError] = useState('');
    // State lokal untuk mengatur status memuat data (untuk disable tombol saat loading).
    const [loading, setLoading] = useState(false);

    // Mengambil fungsi login dari AuthContext.
    const { login } = useAuth();
    // Hook navigasi untuk mengarahkan pengguna ke halaman lain secara programatik.
    const navigate = useNavigate();

    // Fungsi handler yang dipicu ketika form disubmit.
    const handleSubmit = async (e) => {
        e.preventDefault(); // Mencegah reload halaman secara default oleh browser.
        setError(''); // Mereset pesan error sebelumnya.
        setLoading(true); // Mengubah state loading menjadi true.
        ...
        try {
            ...
            if (result.success) {
                // Jika login sukses, arahkan pengguna ke rute daftar posting (/posts).
                navigate('/posts');
            } else {
                // Jika gagal, set pesan error berdasarkan respons API.
            ...
            setLoading(false); // Mengembalikan state loading ke false setelah proses selesai.
        }
    };
