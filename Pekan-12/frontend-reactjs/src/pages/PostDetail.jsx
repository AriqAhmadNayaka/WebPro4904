...
// Komponen Halaman untuk Menampilkan Detail Artikel Lengkap.
const PostDetail = () => {
    // Mengambil parameter `id` artikel dari URL (misalnya dari /posts/5, didapatkan id = 5).
    const { id } = useParams();
    // State untuk menyimpan objek data detail artikel yang berhasil diambil.
    const [post, setPost] = useState(null);
    // State loading untuk menampilkan indikator pemuatan.
    const [loading, setLoading] = useState(true);
    // State error untuk menampilkan pesan kesalahan.
    const [error, setError] = useState('');

    // Memanggil fetchPost setiap kali parameter `id` di URL berubah.
    useEffect(() => {
        fetchPost();
    }, [id]);

    // Fungsi asinkronus untuk mengambil satu data artikel berdasarkan ID.
    const fetchPost = async () => {
        try {
            ...
            // Memberikan pesan error spesifik jika artikel tidak ditemukan di server (status 404).
    ...
    // Fungsi pembantu untuk memformat tanggal artikel menjadi format tanggal standar Indonesia (contoh: 21 Mei 2026).
