...
// Komponen navbar yang selalu tampil di atas semua halaman
const Navbar = () => {
    const { user, logout } = useAuth(); // ambil data user dan fungsi logout dari context
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false); // state untuk buka/tutup menu mobile

    // Fungsi logout dipanggil saat tombol Logout diklik
        ...
        <nav className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
            <div className="max-w-screen-xl mx-auto px-4">
                <div className="flex items-center justify-between h-16">
                    ...
                    {/* Logo kiri - klik ke halaman posts */}
                    ...
                    {/* Menu navigasi tengah - hanya tampil di layar md ke atas */}
                    ...
                    {/* Bagian kanan: avatar, nama user, tombol logout, dan hamburger */}
                    <div className="flex items-center space-x-4">

                        {/* Avatar dan nama user - tersembunyi di layar kecil */}
                        <div className="hidden sm:flex items-center space-x-3">
                            <DefaultAvatar className="w-8 h-8" />
                            <span className="text-sm font-medium text-gray-900">
                                {user?.name || 'User'} {/* fallback ke 'User' jika nama kosong */}
                            </span>
                        </div>
                ...
                {/* Menu mobile - tampil hanya jika mobileMenuOpen = true */}
                {mobileMenuOpen && (
