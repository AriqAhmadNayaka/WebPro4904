...
const PostList = () => {
    ...
    const fetchPosts = async () => {
        try {
            setLoading(true);
            const response = await postsAPI.getAll(); // Request semua post dari API
        ...
        <div className="min-h-screen bg-gray-100 flex flex-col">
            ...
            <main className="flex-1">
                <div className="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                    ...
                    {/* Kondisi render: loading → error → kosong → daftar post */}
                    {loading ? (
                        ...
                        // Tampilkan error dan tombol retry jika request gagal
                        ...
                        // Tampilkan pesan jika belum ada post sama sekali
                        <div className="text-center py-20">
                            <p className="text-gray-500">Belum ada posts.</p>
                        </div>
                    ) : (
                        // Render grid kartu post, 2 kolom di layar besar
                        <div className="grid gap-8 lg:grid-cols-2">
                            {posts.map((post) => (
                                <PostCard key={post.id} post={post} /> // key unik mencegah re-render berlebih
                            ))}
                        </div>
                    )}
                </div>
