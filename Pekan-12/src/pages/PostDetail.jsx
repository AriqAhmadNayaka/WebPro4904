...
const PostDetail = () => {
    const { id } = useParams(); // Ambil ID post dari URL
    const [post, setPost] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');

    // Fetch ulang data setiap kali ID di URL berubah
    useEffect(() => {
        fetchPost();
    }, [id]);
    ...
    const fetchPost = async () => {
        try {
            setLoading(true);
            const response = await postsAPI.getById(id); // Request ke API berdasarkan ID
            setPost(response.data.data);
            setError('');
        } catch (err) {
            // Bedakan error 404 (tidak ditemukan) dengan error lainnya
    ...
    // Format tanggal ke format lokal Indonesia, misal: "12 Mei 2024"
    ...
    // Tampilkan pesan error beserta tombol kembali jika gagal memuat
    if (error) {
        return (
        ...
        <div className="min-h-screen bg-gray-100 flex flex-col">
            ...
            <main className="flex-1">
                <div className="py-8 px-4 mx-auto max-w-screen-lg lg:py-16 lg:px-6">
                    {/* Tombol navigasi kembali ke halaman daftar post */}
                    <div className="mb-6">
                        <Link
                            to="/posts"
                    ...
                    <article className="bg-white rounded-lg border border-gray-200 shadow-md overflow-hidden">
                        {/* Gambar header hanya ditampilkan jika tersedia */}
                        {post.image_url && (
                            <img
                                src={post.image_url}
                                alt={post.title}
                        ...
                        <div className="p-6 lg:p-10">
                            {/* Meta info: kategori, penulis, tanggal */}
                            ...
                            {/* Render isi artikel, setiap baris baru dijadikan paragraf terpisah */}
