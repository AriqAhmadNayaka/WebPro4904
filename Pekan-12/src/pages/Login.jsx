...
const Login = () => {
    ...
    const [error, setError] = useState('');   // Pesan error dari server atau jaringan
    const [loading, setLoading] = useState(false); // Status proses login berlangsung
    ...
    const handleSubmit = async (e) => {
        ...
        setError('');        // Reset pesan error sebelum mencoba login
        setLoading(true);    // Aktifkan indikator loading

        try {
            const result = await login(email, password); // Kirim request login ke API

            if (result.success) {
                navigate('/posts'); // Redirect ke halaman daftar post jika berhasil
            } else {
                setError(result.message); // Tampilkan pesan error dari server
            }
        } catch (err) {
            // Tangani error tak terduga, misal server mati atau network error
                ...
                <div className="w-full max-w-md bg-white rounded-xl shadow-lg border border-gray-200 p-8">
                    ...
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        <div>
                            ...
                            {/* Input email dikontrol dengan state */}
                            <input
                                type="email"
                                name="email"
                                id="email"
                        ...
                        <div>
                            ...
                            {/* Input password dikontrol dengan state */}
                            <input
                                type="password"
                                name="password"
                                id="password"
                        ...
                        {/* Tombol dinonaktifkan saat proses login berlangsung */}
                        <button
                            type="submit"
                            disabled={loading}
