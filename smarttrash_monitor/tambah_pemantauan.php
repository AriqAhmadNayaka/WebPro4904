...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($lokasi === '' || $waktuLaporan === '' || $jumlahKendaraanInput === '' || $deskripsi === '') {
        ...
        $error = 'Berat sampah harus berupa angka dan minimal 1.';
        ...
        if ($fileError !== UPLOAD_ERR_OK) {
            ...
            if (!move_uploaded_file($fileTmp, $uploadPath)) {
                ...
                $stmt = mysqli_prepare($koneksi, 'INSERT INTO monitor (user_id, lokasi_tps, waktu_pemantauan, berat_sampah, status_kapasitas, deskripsi, foto_bukti) VALUES (?, ?, ?, ?, ?, ?, ?)');
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    set_flash('success', 'Pemantauan berhasil ditambahkan.');
                ...
                $error = 'Data pemantauan gagal disimpan.';
            }
        }
    }
}
...
<head>
    ...
    <title>Tambah Pemantauan | SmartTrash Monitoring</title>
...
<body>
    ...
    <main class="container page-layout">
        <section class="page-intro">
            <p class="eyebrow">Input Pemantauan</p>
            <h2>Tambah data pemantauan sampah</h2>
            <p>Status kapasitas dihitung otomatis dengan logika if/else berdasarkan berat sampah.</p>
        </section>

        <section class="form-card wide-card">
            <h3>Form Tambah Pemantauan</h3>
            ...
            <form method="POST" enctype="multipart/form-data" class="form-grid">
                <label>
                    <span>Lokasi</span>
                    <input type="text" name="lokasi_tps" required value="<?= htmlspecialchars($_POST['lokasi_tps'] ?? ''); ?>" placeholder="Contoh: TPS Kebun Hadil">
                </label>

                <label>
                    <span>Waktu Pemantauan</span>
                    <input type="datetime-local" name="waktu_pemantauan" required value="<?= htmlspecialchars($_POST['waktu_pementauan'] ?? ''); ?>">
                </label>

                <label>
                    <span>Berat Sampah</span>
                    <input type="number" name="berat_sampah" min="1" required value="<?= htmlspecialchars($_POST['berat_sampah'] ?? ''); ?>">
                </label>
                ...
                <button type="submit" class="btn btn-primary">Simpan Pemantauan</button>
            </form>
        </section>
    </main>
