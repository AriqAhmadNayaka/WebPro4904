...
// Halaman Login (Masuk Log)
// Menyediakan form login pengguna dengan validasi dan interaksi status loading serta penanganan error.
const Login = () => {
  ...
  const [error, setError] = useState(''); // State untuk menyimpan pesan kesalahan/error login
  const [loading, setLoading] = useState(false); // State untuk melacak status pemuatan asinkron saat submit form
  
  // Mengambil fungsi login dari useAuth context
  const { login } = useAuth();
  // Navigasi Router DOM untuk mengalihkan rute secara terprogram
  const navigate = useNavigate();

  // Menangani proses submit form
  const handleSubmit = async (e) => {
    e.preventDefault(); // Mencegah reload halaman bawaan browser saat submit form
    setError(''); // Bersihkan pesan kesalahan sebelumnya
    setLoading(true); // Mulai pemuatan

    try {
      // Memanggil fungsi login asinkron dari AuthContext
      const result = await login(email, password);
      
      if (result.success) {
        // Jika login berhasil, alihkan pengguna ke halaman daftar postingan terproteksi
        navigate('/posts');
      } else {
        // Jika login gagal (misal kredensial salah), tampilkan pesan kegagalan dari API
        setError(result.message);
      }
    } catch (err) {
      console.error('Login error:', err);
      // Fallback jika terjadi error jaringan atau server mati
      setError(err.message || 'Terjadi kesalahan saat login. Pastikan server Laravel/CI3 sudah berjalan.');
    } finally {
      setLoading(false); // Matikan pemuatan
    }
  };

  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      {/* Bagian Konten Utama: Terpusat secara vertikal & horisontal */}
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-8">
        ...
        <div className="w-full max-w-md bg-white rounded-xl shadow-lg border border-gray-200 p-8">
          ...
          {/* Alert Penampil Kesalahan (Hanya tampil jika ada pesan error) */}
          {error && (
      ...
      {/* Footer Hak Cipta Bawah Halaman */}
