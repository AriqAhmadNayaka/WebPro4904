...
class Auth extends MX_Controller {
    ...
    public function register()
    {
        ...
        try {
            ...
            $user_data = array(
                // Membersihkan nama dari karakter HTML dan menyimpannya sebagai username.
                ...
                // Peran default sesuai skema ci3_users.
                'role' => 'sekolah'
            );
            ...
            $token = $this->jwt->create(array(
                ...
                'id' => $user->id,
                // Menyimpan nama user ke payload token (diambil dari username).
    ...
    // Endpoint debug: melihat semua user yang sudah terdaftar di database.
    public function users()
    {
        $this->_json();
        $users = $this->User_model->get_all();
        return $this->_response(true, 'Daftar user terdaftar', $users);
    }

    // Endpoint debug: menghapus semua data user untuk reset testing.
    // HANYA untuk keperluan development/testing — HAPUS di production!
    public function reset()
    {
        $this->_json();
        $this->db->truncate('users');
        return $this->_response(true, 'Semua data user berhasil dihapus. Silakan register ulang.');
    }
    ...
    public function me()
    {
        ...
        return $this->_response(true, 'User detail', array(
            // ID user.
            'id' => $user->id,
            // Nama user (diambil dari username).
            'name' => isset($user->username) ? $user->username : $user->name,
            // Email user.
            'email' => $user->email,
            // Waktu user dibuat.
