<?php
// Memulai sesi agar bisa memeriksa status pengguna
session_start();

// Memanggil file koneksi database
require 'koneksi.php';

// PROTEKSI HALAMAN: Jika user belum login (session 'username' kosong), maka kembali ke login.php
if (!isset($_SESSION['username'])) {
    header("Location: login.php");
    exit();
}

// Mengecek apakah form disubmit (POST) DAN tombol dengan name 'tambah_manual' ditekan
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['tambah_manual'])) {
    
    // 1. Mengambil data teks dari form
    $tgl = $_POST['tanggal'];
    $ket = $_POST['keterangan'];
    $jenis = $_POST['jenis'];
    $jumlah = $_POST['jumlah'];

    // 2. Mengambil informasi file gambar (Upload)
    $nama_file = $_FILES['bukti']['name']; // Mengambil nama asli file gambar
    $tmp_file = $_FILES['bukti']['tmp_name']; // Mengambil lokasi penyimpanan sementara di memori server
    
    // Menentukan lokasi folder tujuan
    $path = "uploads/" . $nama_file; 

    // Memindahkan file gambar dari memori sementara (tmp_name) ke folder 'uploads' di komputer kita
    if (move_uploaded_file($tmp_file, $path)) {
        
        // 3. Jika gambar BERHASIL dipindah, Simpan gabungan data teks & nama file ke Database (CRUD - Create)
        $stmt = $conn->prepare("INSERT INTO keuangan (tanggal, keterangan, jenis, jumlah, bukti_transaksi) VALUES (?, ?, ?, ?, ?)");
        
        // sssis = string, string, string, integer, string (mengamankan data yang masuk)
        $stmt->bind_param("sssis", $tgl, $ket, $jenis, $jumlah, $nama_file);
        
        // Menjalankan query insert ke database
        if ($stmt->execute()) {
            // Alert berhasil dan merefresh halaman dompet.php
            echo "<script>alert('Data dan Bukti Transaksi berhasil disimpan!'); window.location='dompet.php';</script>";
            exit();
        } else {
             // Alert jika data gagal masuk tabel database
             echo "<script>alert('Gagal menyimpan ke database!');</script>";
        }
    } else {
        // Alert jika gambar gagal dipindahkan ke folder uploads
...
<body>
  ...
  <main class="main">
    ...
    <section id="section-laporan-manual" style="margin-top: 24px; scroll-margin-top: 80px">
      ...
      <form id="formManual" method="POST" action="" enctype="multipart/form-data">
        ...
        <input type="file" name="bukti" accept="image/*" required>
          ...
          <button type="submit" name="tambah_manual" style="margin-top: 15px;">Simpan Data & Upload File</button>
        ...
        <div class="table-card" style="flex:1;">
          ...
          <tbody>
              ...
              if ($result->num_rows > 0) {
                  ...
                  echo "<tr><td colspan='4'>Belum ada data transaksi</td></tr>";
              }
              ?>
            </tbody>
          </table>
  ...
  <script>
    function hitungRekap() {
      const rows = document.querySelectorAll("#tabelKeuangan tbody tr");
    ...
    document.getElementById("formManual").addEventListener("submit", function (e) {
      ...
      if (!tgl || !ket || !jumlah) return;

      const tbodyManual = document.querySelector("#tabelManual tbody");
      const trManual = document.createElement("tr");
      trManual.innerHTML = `<td>${tgl}</td><td>${ket}</td><td>${jenis}</td><td>${jumlah}</td>`;
      tbodyManual.appendChild(trManual);

      const tbodyKeu = document.querySelector("#tabelKeuangan tbody");
      const trKeu = document.createElement("tr");
      trKeu.innerHTML = `<td>${tgl}</td><td>${ket}</td><td>${jenis}</td><td>${jumlah}</td>`;
      tbodyKeu.appendChild(trKeu);
