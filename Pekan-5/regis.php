<?php
include("Koneksi.php"); // menambahkan ini agar bisa konek ke database

//untuk menyimpan pesan yang akan di ketik user
$message = "";
$type = "";

$role = "";
$name = "";
$email = "";

//memastika kode di jalankan setelah pengguna mengisi form
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $role     = $_POST['role'] ?? "";
    $name     = trim($_POST['name'] ?? "");
    $email    = trim($_POST['email'] ?? "");
    $password = trim($_POST['password'] ?? "");

    //memvalidasi kekosongan data
    if ($role == "" || $name == "" || $email == "" || $password == "") {
        $message = "Semua field wajib diisi!";
        $type = "danger";
    }
    //mencek panjang variabel
    elseif (strlen($name) < 3) {
        $message = "Nama minimal 3 karakter!";
        $type = "danger";
    }
    //mencek nama hanya boleh alfabet
    elseif (!preg_match("/^[a-zA-Z ]*$/", $name)) {
        $message = "Nama tidak boleh mengandung angka atau simbol!";
        $type = "danger";
    }
    //mencek alamat email apa bukan
    elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $message = "Format email tidak valid!";
        $type = "danger";
    }
    //mencek password harus berisi 6 karakter
    elseif (strlen($password) < 6) {
        $message = "Password minimal 6 karakter!";
        $type = "danger";
    }
    else {
        
        // menyimpan data pengguna ke database
        $sql = "INSERT INTO user1 (nama, email, password) VALUES (?, ?, ?)";
        $stmt = $conn->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("sss", $name, $email, $password);            
            //jika berhasil pindah ke halaman login
            if ($stmt->execute()) {
                $message = "Akun berhasil dibuat! Silakan login.";
                $type = "success";
                header("refresh:2;url=login.php");
            } else {
                $message = "Gagal menyimpan ke database: " . $conn->error;
                $type = "danger";
            }
            $stmt->close();
        } else {
            $message = "Kesalahan sistem: " . $conn->error;
            $type = "danger";
        }
    }
}
?>

<!DOCTYPE html>
<html lang="id">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>LifeTrack - Buat Akun</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <style>
        body {
            background: #f4f7f6;
            min-height: 100vh;
            display: flex;
            flex-direction: column;
        }
        .signin-wrapper {
            flex: 1;
            display: flex;
            justify-content: center;
            align-items: center;
            padding: 20px 0;
        }
        .signin-card {
            border-radius: 18px;
            box-shadow: 0 4px 20px rgba(0,0,0,0.08);
            background: white;
        }
        .btn-lifetrack {
            background: #0f766e;
            border: none;
            padding: 10px;
        }
        .btn-lifetrack:hover {
            background: #0d5f57;
            color: white;
        }
        .text-lifetrack {
            color: #0f766e;
        }
        footer {
            background: #0f766e;
            color: white;
            text-align: center;
            padding: 24px;
        }
    </style>
</head>

<body>
<div class="signin-wrapper">
    <div class="card signin-card p-4" style="width:360px;">

        <h4 class="fw-bold text-lifetrack mb-1">Buat Akun</h4>
        <p class="text-muted mb-4">Daftar untuk mulai menggunakan LifeTrack</p>

        <?php if ($message != "") : ?>
            <div class="alert alert-<?php echo $type; ?> py-2 small">
                <?php echo $message; ?>
            </div>
        <?php endif; ?>

        <form method="POST" action="">
            <div class="mb-3">
                <label class="form-label fw-semibold">Daftar sebagai</label>
                <select name="role" class="form-select">
                    <option value="">Pilih peran</option>
                    <option value="pasien" <?php if($role == "pasien") echo "selected"; ?>>Pasien</option>
                    <option value="tenaga_medis" <?php if($role == "tenaga_medis") echo "selected"; ?>>Tenaga Medis</option>
                </select>
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Nama Lengkap</label>
                <input
                    type="text"
                    name="name"
                    class="form-control"
                    placeholder="Masukkan nama lengkap"
                    value="<?php echo htmlspecialchars($name); ?>"
                >
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Email</label>
                <input
                    type="email"
                    name="email"
                    class="form-control"
                    placeholder="Masukkan email"
                    value="<?php echo htmlspecialchars($email); ?>"
                >
            </div>

            <div class="mb-3">
                <label class="form-label fw-semibold">Password</label>
                <input
                    type="password"
                    name="password"
                    class="form-control"
                    placeholder="Buat password"
                >
            </div>

            <button type="submit" class="btn btn-lifetrack text-white w-100 fw-bold mt-2">
                Buat Akun
            </button>
        </form>

        <div class="text-center mt-3">
            <small>
                Sudah punya akun?
                <a href="login.php" class="text-lifetrack fw-bold text-decoration-none">
                    Login
                </a>
            </small>
        </div>

    </div>
</div>

<footer>
    LifeTrack © 2025 — Membantu Anda hidup lebih sehat.
</footer>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>