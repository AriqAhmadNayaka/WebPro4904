...
$ROLE_REDIRECT = [
    ...
    // role di bawah belum tersedia, jadi hanya bisa pilih role sekolah untuk login berhasil
    // 'orangtua' => './orangtua/dashboard/dashboardOrtu.php',
    // 'dinas'    => './instansi/dinas.php',
    // 'admin'    => './admin/dashboard/dashboardAdmin.php',
];

// require koneksi.php agar variabel $conn tersedia di seluruh halaman ini
// require dipilih karena koneksi database adalah komponen wajib
// jika file tidak ditemukan, program langsung berhenti (fatal error)
require 'koneksi.php';

// Membersihkan input dari spasi berlebih, backslash, dan karakter berbahaya atau XSS
...
// CEK COOKIE "INGAT SAYA" (sesuai modul 5.4.7)
// isset($_COOKIE[...]) mengecek apakah cookie sudah ada di browser user
// Jika ada, isi variabel untuk mengisi form login otomatis
$cookieEmail = isset($_COOKIE['inklu_email']) ? $_COOKIE['inklu_email'] : '';
$cookieRole  = isset($_COOKIE['inklu_role'])  ? $_COOKIE['inklu_role']  : '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'register') {
        ...
        // preg_match: password harus mengandung minimal 1 karakter spesial
            ...
            // Cek apakah email sudah terdaftar di database menggunakan SELECT
            // prepare() + bind_param() dipakai agar query aman dari SQL injection
            $cekEmail = $conn->prepare("SELECT id FROM users WHERE email = ?");

            // "s" artinya parameter bertipe string, sesuai tabel types di modul 4.4.2
            ...
            if ($cekEmail->num_rows > 0) {
                ...
                // password_hash() agar password tidak tersimpan sebagai teks biasa di database
                $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

                // INSERT data user baru ke tabel users menggunakan prepared statement
                // ? sebagai placeholder yang akan diisi oleh bind_param di bawah
                $sql  = "INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)";
                $stmt = $conn->prepare(query: $sql);
                // "ssss" = 4 parameter bertipe string sesuai urutan kolom di query
                ...
                if ($stmt->execute()) {
                    ...
                    $notification      = 'Registrasi gagal, silakan coba lagi';
                    $notification_type = 'error';
                }
                $stmt->close();
            }
    ...
    // LOGIN
        ...
        if (empty($email) || empty($password) || empty($role)) {
            ...
            // Cari user berdasarkan email dan role menggunakan SELECT + prepared statement
            ...
            $stmt->execute();
            $stmt->store_result();
            // bind_result() mengikat hasil query ke variabel PHP
            $stmt->bind_result($userId, $userName, $userEmail, $userPassword, $userRole);
            $stmt->fetch();

            // password_verify() mencocokkan password input dengan hash yang tersimpan di database
            if ($stmt->num_rows === 0 || !password_verify($password, $userPassword)) {
                ...
                // COOKIE "INGAT SAYA" (sesuai modul 5.4.6) 
                // setcookie() harus dipanggil sebelum ada output HTML apapun
                // time() + (86400 * 30) = kedaluwarsa 30 hari dari sekarang
                if (isset($_POST['ingat_saya'])) {
                    // Simpan email dan role ke cookie selama 30 hari
                    setcookie("inklu_email", $userEmail, time() + (86400 * 30), "/");
                    setcookie("inklu_role",  $userRole,  time() + (86400 * 30), "/");
                } else {
                    // Jika tidak dicentang, hapus cookie dengan set expire ke masa lalu
                    // sesuai modul 5.4.8: cookie dihapus dengan expire time() - 3600
                ...
                $stmt->close();
                $conn->close();
                header("Location: " . ($ROLE_REDIRECT[$userRole] ?? '#'));
                exit;
            }
            $stmt->close();
        }
            ...
            <form id="loginForm" class="auth-form" method="POST" action="auth.php">
                ...
                <div class="form-group">
                    ...
                    <div class="input-wrapper">
                        ...
                        <!-- value diisi dari cookie jika user pernah memilih "Ingat Saya"
                             Sesuai modul 5.4.7: akses nilai cookie menggunakan $_COOKIE["nama"] -->
                ...
                <div class="form-group">
                    ...
                    <div class="select-wrapper">
                        <!-- selected diisi dari cookie jika user pernah memilih "Ingat Saya"
                             Sesuai modul 5.4.7: akses nilai cookie menggunakan $_COOKIE["nama"] -->
                ...
                <!-- Checkbox "Ingat Saya" — jika dicentang, email dan role disimpan ke cookie
                     Sesuai modul 5.4.6: cookie dibuat menggunakan setcookie() di PHP
                     Checkbox otomatis tercentang jika cookie sudah ada dari login sebelumnya -->
