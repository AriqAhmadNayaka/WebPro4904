...
/**
 * Logika Penambahan Dokter Baru (Create)
...
if (isset($_POST['tambah'])) {
  ...
  $bio = $_POST['bio'];
  $dokumen = $_FILES['sertifikat']; // Mengambil data file yang diupload

  // Memanggil helper uploadFile untuk memvalidasi dan menyimpan file sertifikat
  $sertifikat = uploadFile($dokumen, 'uploads/');
  
  if ($sertifikat['status'] == "success") {
    $data = $_POST;
    unset($data['tambah']); // Menghapus key submit dari array data
    $data['sertifikat'] = $sertifikat['filename']; // Menyimpan nama file hasil upload
    
    try {
      // Menyisipkan data ke database menggunakan helper insertData
      insertData($conn, "dokter", $data);
      $success_msg = "Dokter berhasil ditambahkan!";
    } catch (mysqli_sql_exception) {
      $error_msg = "Error! pastikan email dan kode dokter belum terdaftar!";
    }
  } else {
    // Jika upload file gagal, simpan pesan errornya
    $error_msg = "Gagal upload sertifikat: " . $sertifikat['msg'];
  }
}
?>
