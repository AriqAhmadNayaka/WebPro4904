...
// Mengecek apakah session user_id sudah ada, jika tidak ada maka pengguna akan diarahkan ke halaman login
if (!isset($_SESSION['user_id'])) {

    if (isset($_COOKIE['user_id']) && isset($_COOKIE['nama'])) {
...
// Mengecek apakah form laporan manual telah disubmit dengan metode POST
// POST digunakan untuk mengirim data dari form ke server, data yang dikirim tidak akan terlihat di URL
// Jika form disubmit, maka data akan diproses untuk disimpan ke database
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Mengecek apakah ada file foto yang diupload, jika ada maka file akan diproses untuk disimpan di server dan nama file akan disimpan di database
    // File foto akan disimpan di folder "uploads" dengan nama asli file yang diupload
    // Validasi format file foto yang diupload, hanya mengizinkan format JPG, JPEG, dan PNG
    // Jika format file tidak sesuai, maka akan menampilkan pesan error
    // Jika file berhasil diupload, maka nama file akan disimpan di variabel $foto untuk disimpan di database
    // Fungsi move_uploaded_file() digunakan untuk memindahkan file yang diupload ke folder tujuan di server
    ...
    // Mengecek apakah ID laporan sudah ada, jika ada maka data akan diupdate, jika tidak ada maka data akan disimpan sebagai laporan baru
    // Jika ID laporan sudah ada, maka query UPDATE akan dijalankan untuk memperbarui data laporan yang sudah ada di database
    // Jika ID laporan tidak ada, maka query INSERT akan dijalankan untuk menyimpan data laporan baru ke database
    // Fungsi prepare() digunakan untuk mempersiapkan query SQL dengan parameter yang akan diisi kemudian menggunakan bind_param() untuk mengikat parameter dengan nilai yang akan disimpan
    if (!empty($id)) {
        // UPDATE
        if (!empty($foto)) {
            $stmt = $conn->prepare("UPDATE laporan SET tanggal=?, keterangan=?, jenis=?, jumlah=?, foto=? WHERE id=? AND user_id=?");
            $stmt->bind_param("sssisii", $tanggal, $keterangan, $jenis, $jumlah, $foto, $id, $user_id);
        } else {
            $stmt = $conn->prepare("UPDATE laporan SET tanggal=?, keterangan=?, jenis=?, jumlah=? WHERE id=? AND user_id=?");
            $stmt->bind_param("sssiii", $tanggal, $keterangan, $jenis, $jumlah, $id, $user_id);
        }

    } else {
        // INSERT
        $stmt = $conn->prepare("INSERT INTO laporan (user_id, tanggal, keterangan, jenis, jumlah, foto) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssis", $user_id, $tanggal, $keterangan, $jenis, $jumlah, $foto);
    }
    ...
    // Mengecek apakah ada parameter 'hapus' di URL menggunakan metode GET, jika ada maka data laporan dengan ID yang sesuai akan dihapus dari database
    if (isset($_GET['hapus'])) {
        $id = $_GET['hapus'];
...
// Mengambil data laporan dari database untuk ditampilkan di halaman, hanya data laporan yang sesuai dengan user_id yang sedang login yang akan diambil
$data = mysqli_query($conn, "SELECT * FROM laporan WHERE user_id = $user_id");

// Mengecek apakah ada parameter 'edit' di URL menggunakan metode GET, jika ada maka data laporan dengan ID yang sesuai akan diambil dari database untuk ditampilkan di form edit
$editData = null;
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    ...
    <section id="section-laporan-manual" style="margin-top: 24px; scroll-margin-top: 80px">
      ...
      <div class="row">
        ...
        <div class="table-card" style="flex:1;">
          ...
          $query = "SELECT * FROM laporan WHERE user_id = ".$_SESSION['user_id'];
          $result = mysqli_query($conn, $query);
          // Perulangan untuk menampilkan setiap baris data laporan yang diambil dari database, menggunakan mysqli_fetch_assoc untuk mendapatkan data dalam bentuk array asosiatif
          while($row = mysqli_fetch_assoc($result)):?>
          <!-- Menampilkan data laporan pada tabel HTML, dengan menggunakan htmlspecialchars untuk mencegah serangan XSS dengan mengubah karakter khusus menjadi entitas HTML -->
          <tr>
