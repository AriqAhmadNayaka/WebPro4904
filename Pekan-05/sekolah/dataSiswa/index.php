...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'simpan') {
        ...
        // filter_var: validasi format email jika diisi
            ...
            // HANDLE UPLOAD FOTO (sesuai modul 5.4.9)
            // Folder penyimpanan foto siswa di dalam folder dataSiswa
            $uploadDir    = __DIR__ . '/uploads/';
            $namaFotoBaru = null; // null berarti tidak ada foto baru yang diunggah
            ...
            if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
                ...
                // Daftar format gambar yang diizinkan sesuai modul 5.4.9
                $allowedTypes = ['jpg', 'jpeg', 'png'];

                // getimagesize() memverifikasi bahwa file benar-benar gambar, bukan file lain yang disamarkan
                $check = getimagesize(filename: $_FILES['foto']['tmp_name']);

                if ($check === false || !in_array(needle: $imageFileType, haystack: $allowedTypes)) {
                    $notification      = 'Format foto tidak valid. Gunakan JPG, JPEG, atau PNG';
                    $notification_type = 'error';
                    goto endSimpan;
                }
                ...
                // Hindari duplikasi nama file dengan penomoran otomatis seperti contoh di modul 5.4.9
                ...
                // Nama file baru = angka tertinggi + 1, contoh: 1.jpg, 2.png, dst
                $namaFotoBaru = ($highestNumber + 1) . '.' . $imageFileType;
                $targetFile   = $uploadDir . $namaFotoBaru;

                // move_uploaded_file() memindahkan file dari folder sementara ke folder uploads
                if (!move_uploaded_file(from: $_FILES['foto']['tmp_name'], to: $targetFile)) {
                    $notification      = 'Gagal mengupload foto';
                    $notification_type = 'error';
                    goto endSimpan;
                }
            }
            ...
            if ($editId > 0) {
                // UPDATE: jika ada foto baru diunggah, hapus foto lama lalu update kolom foto
                // Jika tidak ada foto baru, kolom foto tidak ikut diubah
                if ($namaFotoBaru !== null) {
                    // Ambil nama foto lama untuk dihapus dari folder
                    $stmtFoto = $conn->prepare("SELECT foto FROM siswa WHERE id = ?");
                    $stmtFoto->bind_param("i", $editId);
                    $stmtFoto->execute();
                    $stmtFoto->bind_result($fotoLama);
                    $stmtFoto->fetch();
                    $stmtFoto->close();

                    // Hapus file foto lama dari folder jika ada
                    if ($fotoLama && file_exists($uploadDir . $fotoLama)) {
                        unlink($uploadDir . $fotoLama);
                    }

                    // UPDATE dengan foto baru — "sssssssssi" = 9 string + 1 integer (id)
                    $sql  = "UPDATE siswa SET nisn=?, nama=?, kelas=?, jenis_kelamin=?, email=?, no_telepon=?, alamat=?, status=?, foto=? WHERE id=?";
                    $stmt = $conn->prepare(query: $sql);
                    $stmt->bind_param("sssssssssi", $nisn, $nama, $kelas, $jenisKelamin, $email, $noTelepon, $alamat, $status, $namaFotoBaru, $editId);
                } else {
                ...
                // CREATE: INSERT siswa baru beserta nama foto jika ada
                // "sssssssss" = 9 string (kolom foto bisa NULL jika tidak ada upload)
                $sql  = "INSERT INTO siswa (nisn, nama, kelas, jenis_kelamin, email, no_telepon, alamat, status, foto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare(query: $sql);
                $stmt->bind_param("sssssssss", $nisn, $nama, $kelas, $jenisKelamin, $email, $noTelepon, $alamat, $status, $namaFotoBaru);
                $stmt->execute();
                $stmt->close();
            ...
            endSimpan: // Label tujuan goto jika validasi foto gagal
        }
    }
}
    ...
    <div class="card">
      <table>
        ...
        <tbody>
          ...
          <tr>
            <!-- Tampilkan foto siswa jika ada, atau ikon default jika belum punya foto -->
            <td>
              <?php if (!empty($s['foto'])): ?>
...
<div class="modal" id="studentModal">
  <div class="modal-content">
    ...
    <!-- Form dikirim via POST dengan enctype multipart/form-data agar bisa mengirim file
         sesuai materi modul 5.4.9: form upload file wajib pakai enctype="multipart/form-data" -->
    <form id="studentForm" method="POST" action="index.php" enctype="multipart/form-data">
      ...
      <!-- Input upload foto siswa — accept="image/*" membatasi pilihan hanya file gambar
           sesuai tabel accept di modul 5.4.9 -->
      <div style="margin-top:12px;">
        ...
        <!-- Preview foto yang sudah tersimpan (ditampilkan saat edit) -->
        ...
        <!-- Input file hanya menerima format gambar sesuai modul 5.4.9 -->
...
<script>
  ...
  function openModal() {
    ...
    // Sembunyikan preview foto saat tambah baru
  ...
  // Parameter foto berisi nama file foto yang tersimpan di folder uploads
  function openEditModal(id, nisn, nama, kelas, jk, email, telp, alamat, status, foto) {
    ...
    // Tampilkan foto yang sudah tersimpan jika ada, sembunyikan jika belum punya foto
    if (foto && foto !== '') {
  ...
  // Tutup modal dan reset semua field form termasuk preview foto
  function closeModal() {
  ...
  // Tampilkan preview foto baru yang dipilih user sebelum form disubmit
