...
<html lang="id">
  ...
  <body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["nama"]) && isset($_POST["email"]) && isset($_POST["password"]) && isset($_POST["confirm_password"]) && isset($_POST["regRole"])){
      ...
      // Validasi input
      if (empty($nama) || empty($email) || empty($password) || empty($confirm_password)) { //nama dan email sudah dicek required di form, tapi tetap dicek di sini untuk keamanan
        echo "Semua field harus diisi.";
      } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) { //validasi format email
        echo "Format email tidak valid.";
      } elseif ($password !== $confirm_password) { //validasi kecocokan password dan konfirmasi password
        echo "Password dan konfirmasi password tidak cocok.";
      } else {// Simpan data ke file users.txt (format: nama|email|password|role)
    ...
    //ini script dari semester 1, jadi ga diubah
